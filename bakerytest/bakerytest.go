...
func (d *InteractiveDischarger) wait(w http.ResponseWriter, r *http.Request) {
	...
	check := bakery.ThirdPartyCaveatCheckerFunc(func(cav *bakery.ThirdPartyCaveatInfo) ([]checkers.Caveat, error) {
		return cavs, nil
	})
	m, cavs, err := bakery.Discharge(d.Key, check, discharge.cavId)
	if err != nil {
		code, body := httpbakery.ErrorToResponse(err)
		httprequest.WriteJSON(w, code, body)
		return
